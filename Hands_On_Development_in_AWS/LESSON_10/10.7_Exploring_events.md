...
![enent_notifications.jpg](./IMAGES/10.7_Exploring_events/enent_notifications.jpg)
4. Configure the event:
   - **Event name:** e.g., `ImageUploadEvent`
   - **Filter (suffix):** `.jpg` or `.png` — this ensures only images trigger the function.
   - **Event type:** *All object create events* (e.g., `PUT`, `POST`).
   - **Destination:** Select **Lambda function**.
![destination.jpg](./IMAGES/10.7_Exploring_events/destination.jpg)
> 🔸 **Tip:**  
...
![create_lamdba.jpg](./IMAGES/10.7_Exploring_events/create_lamdba.jpg)
2. Choose **Blueprints → S3 Events**.
   - Select the blueprint: `s3-get-object (Node.js)`.
3. Configure the function:
   - **Function name:** `S3EventDemo`
   - **Execution role:** Create new role from policy templates → `S3EventRole1234LambdaRole`
...
![see_notification.jpg](./IMAGES/10.7_Exploring_events/see_notification.jpg)

# See the Log File go to CloudWatch
 
 1. Go to the **S3EventDemo** Log Group
    
![CloudWatch.jpg](./IMAGES/10.7_Exploring_events/CloudWatch.jpg)

2. Log Events

![event_log.jpg](./IMAGES/10.7_Exploring_events/event_log.jpg)
